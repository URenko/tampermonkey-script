 ...
 */
    ...
    // @connect      translate.googleapis.com   谷歌翻译
    ...
    const idsExtension = {
        ...
        links: (() => {
            ...
            obj[ids.GOOGLE] = text => {
                ...
                if (hasChineseByRange(text)) {
                    rst = `https://translate.googleapis.com/#view=home&op=translate&sl=auto&tl=en&text=${encodeURIComponent(text)}`;
                } else {
                    rst = `https://translate.googleapis.com/#view=home&op=translate&sl=auto&tl=zh-CN&text=${encodeURIComponent(text)}`;
                }
                return rst;
            };
        ...
        engines: (() => {
            ...
            obj[ids.GOOGLE] = (text, time) => {
                let url = 'https://translate.googleapis.com/translate_a/single?client=gtx&dt=t&dt=bd&dj=1&source=input&hl=zh-CN&sl=auto';
                url += `&tk=${token(text)}`;
                if (hasChineseByRange(text)) {
